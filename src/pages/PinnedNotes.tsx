import { Pin, Calendar, X, PinOff } from "lucide-react";
...
const PinnedNotes = () => {
  ...
  const { data: pinnedNotes = [], isLoading } = useQuery({
    queryKey: ["get-pinnednotes"],
    queryFn: async () => {
  ...
  const handleNoteUnpinning = async (id) => {
    setUnpinningId(id);
    try {
      const response = await ApiClient.patch(`/notes/unpin/${id}`);
      ...
      queryClient.invalidateQueries(["get-pinnednotes"]);
    } catch (error) {
      toast(
        <div className="flex items-center space-x-2">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Your most important notes, always at the top
            </p>
          </div>
        </div>
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-start justify-between">
                    ...
                    <Button
                      ...
                      onClick={() => handleNoteUnpinning(note.id)}
